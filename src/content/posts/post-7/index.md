...
- iOS 端地址：<https://apps.apple.com/app/deepseek-ai-assistant/id6737597349>
- 安卓端地址（官方）：<https://download.deepseek.com/app/>
- 安卓端地址（Google Play）：<https://play.google.com/store/apps/details?id=com.deepseek.chat>

### glm

- 官网地址：<https://chatglm.cn/>
- iOS 端地址：<https://apps.apple.com/app/id6450893458>
- 安卓端地址（官方）：<https://chatglm.cn/download>

### qwen

- 官网地址：<https://chat.qwen.ai/>
- iOS 端地址：<https://qwen.ai/download>
- 安卓端地址（官方）：<https://qwen.ai/download>
- 安卓端地址（Google Play）：<https://play.google.com/store/apps/details?id=com.tongyi.intl>

### kimi

- 官网地址：<https://www.kimi.com/>
- iOS 端地址：<https://apps.apple.com/app/kimi/id6474233312>
- 安卓端地址（官方）：<https://www.kimi.com/en/products/download>
- 安卓端地址（Google Play）：<https://play.google.com/store/apps/details?id=com.moonshot.kimichat>

### gemini

- 官网地址：<https://gemini.google.com/>
- iOS 端地址：<https://apps.apple.com/app/google-gemini/id6477489729>
...
- 官网地址：<https://grok.com/> （或通过 X 平台访问：<https://x.com/> ）
...
- iOS 端地址：<https://www.doubao.com/>
- 安卓端地址（官方）：<https://www.doubao.com/download>

#### 文心一言

- 官网地址：<https://ernie.baidu.com/>
- iOS 端地址：<https://ernie.baidu.com/>
- 安卓端地址（官方）：<https://ernie.baidu.com/>

#### mimo

- 官方体验地址：<https://aistudio.xiaomimimo.com/>
- 官方说明（目前未推出独立 App）：<https://mimo.mi.com/docs/en-US/quick-start/faq/others>

#### minimax（海螺 AI）

- 官网地址：<https://hailuoai.com/>
- iOS 端地址：<https://hailuoai.com/>
- 安卓端地址（官方）：<https://hailuoai.com/>

#### 腾讯元宝

- 官网地址：<https://yuanbao.tencent.com/>
- iOS 端地址：<https://yuanbao.tencent.com/>
...
- 号池模式：对于轻度用户而言，买一个 gpt plus 的账号是 20$，折合 150￥，将之用至上限较难。而号池模式就是多个用户公用一个账号来分摊成本
...
NewAPI 是一款开源的大模型 API 中转分发与管理系统（面板）。市面上绝大多数中转站的后台系统和 UI 界面，本质上都是基于 NewAPI 搭建或二次开发的。

它支持将 OpenAI、Claude、Gemini、国内各大厂商的原生接口统一转换为标准的 OpenAI 兼容格式，并具备用户管理、额度充值、按量/按次计费、渠道轮询与负载均衡等完整功能。
...
使用官方 API 通常需要先注册开发者账号、创建 API Key，再按照官方文档配置请求地址、模型名称和认证方式。下面列出本文提到的厂商中较常用的官方入口；具体价格、模型名称和调用限制请以官方文档为准。
...
- GLM API 文档：<https://docs.bigmodel.cn/cn/api/introduction>
- Kimi API 开放平台：<https://platform.kimi.com/>
- Kimi API 文档：<https://platform.kimi.com/docs/overview>
- 腾讯混元 API 文档：<https://cloud.tencent.com/document/product/1729/101832>
- 腾讯混元 API 概览：<https://cloud.tencent.com/document/product/1729/101848>
- xAI API 文档：<https://api.x.ai/docs/>

### 使用中转站

中转站比较杂，市面上有非常多的中转站，这里就不进行列举了。

需要注意中转站有 “跑路” 的风险，所以需要谨慎选择。
...
- 按次计费
  ...
  需要注意，一定不要把按次算钱的 API 导入 Agent，否则额度会消耗的非常快。

- 按量计费

  按量计费通常按照输入和输出的 Token 数量结算，适合代码生成、长文本处理和 Agent 等调用量不固定的场景。使用前应查看输入、输出、缓存和工具调用分别如何计费，并设置预算或限额。

  由于按量计费的中转站非常多，这里不做推荐。
...
不同厂商的 API 请求格式并不完全相同，但目前最常见的是 OpenAI 兼容格式、Gemini 格式和 Claude 格式。许多中转站会把其他模型包装成 OpenAI 兼容接口，具体字段仍应以服务商文档为准。

- OpenAI API 格式：目前 AI 界的 “世界通用语言”（业内事实标准）。以 `/v1/chat/completions` 为典型路径，采用 `messages: [{"role": "user", "content": "..."}]` 结构。绝大多数第三方客户端（如 Cherry Studio）和中转站默认均采用此格式。
- Gemini API 格式：Google 专有的通信协议，通常使用 `/v1beta/models/...:generateContent` 路径，消息体采用 `contents: [{"parts": [{"text": "..."}]}]` 结构，原生支持超大上下文与多模态文件直传。
- Claude API 格式：Anthropic 官方的原生接口规范，以 `/v1/messages` 为核心，将 `system` 提示词独立于消息列表之外，原生支持 Tools/Artifacts 深度调用，格式严谨、吞吐控制优秀。

国内绝大多数 AI 厂商都采用的是 OpenAI 格式。

#### Agent 格式

- Response 格式：Response 格式是专门用于 Agent 的格式，它需要额外设置 response 的 base url。

- Claude Agent 格式：Claude 本身就是和它自己的对话 API 兼容。
...
- 项目主页：<https://github.com/chevey339/kelivo>

配置过程和 rikkahub 类似，在这里不再赘述。

### 使用 cherry studio

cherry studio 专门给 PC 使用，尤其适用于知识库的检索。

- 官方网站：<https://www.cherryai.com/>
- GitHub 仓库：<https://github.com/CherryHQ/cherry-studio>

![cherry studio 设置](./assets/cherry-set.png)
...
如果只是学习语法、定位报错或理解一段代码，直接在官方网页中提问通常已经够用。需要注意的是，应提供最小可复现代码、完整报错和运行环境，并自行验证 AI 给出的修改。

### 专门的 AI IDE

事实上这类 IDE 在 25 年 09 月的时候还非常流行，但现在逐渐被淘汰了。

不如直接用 VS Code 的插件来使用 Agent。

AI IDE 唯一的好处就是可以使用内敛建议，即 AI 在你写代码的时候给你建议。

初学者可能会沉迷于内敛建议的使用，但是这对你在学习的时候帮助甚微，甚至在开发的时候会打断思路。

目前唯一好用的内敛建议只有 Cursor。

#### trae

字节跳动开发的 AI IDE，接入 seed 模型使用。
...
copilot 是 GitHub 开发的 AI 编程助手，可以说它是 VS Code 的官方插件。
...
antigravity 是 Google 开发的 AI 编程助手，但是效果非常差。

甚至谷歌自己的模型在 antigravity 里发挥都不如 WEB 直接对话。
...
Cursor 是唯一推荐的 AI IDE，但其价格昂贵，依旧不如直接使用 VS Code 的插件。
...
除了基于 IDE 的，现在更常见的是基于 cli 的 Agent，即直接通过终端操控你的工作区文件。
...
- Claude Platform 控制台：<https://platform.claude.com/>

#### grok + cursor

实际上只用下载 cursor 即可，grok 有专门为 cursor 适配一套模型，编号为 composer。
...
实际上上述的所有方式都不如 VS Code 直接装插件来的方便，在 VS Code 的应用商店下载对应 AI 的插件即可，例如在 VS Code 里下载 codex 插件。
...
但是由于格式的不兼容，所以会出现一些奇怪的问题。

可以使用以下工具来解决。

#### 使用 cc switch

如果需要在 Claude Code、Codex、OpenCode 等 CLI Agent 之间切换，可以使用 CC Switch 统一管理不同的供应商、模型和配置。下载时建议只使用下面列出的官方渠道。

- 官网地址：<https://ccswitch.io/>
...
诚然，AI 的编程能力已经非常强大了，但是 AI 对架构的理解远不如人类深刻。

AI 不是万能的，时刻保持警惕，不要让 AI 代替思考。
