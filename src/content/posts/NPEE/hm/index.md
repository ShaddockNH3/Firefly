...
- 在**有限区间上**，导函数有界，原函数一定有界。如果是无限区间的话，可以取 f(x)=x 作为反例。
...
$\mathbf{若\ \lim \frac{f'(x)}{g'(x)} \text{ 存在 (或为 } \infty) \implies 则原极限\ \lim \frac{f(x)}{g(x)} \text{ 必然存在且等于相同的值！}}$
...
最重要的事情是要时刻记住自己在做什么（很多时候化规到最后的条件和原来的条件有差别，此时应该看最后化归的条件，而不是原条件。
...
此外，对于拉格朗日中值定理的题目（包括积分相关的证明），可以适当的从中间取一个点，分区间进行讨论。
...
- **注意！有定积分的前提是 f(x) 在 [a,b] 上有界，如果直接是无界的，那么就没有定积分！**
...
### 积分相关的计算步骤（不管是二重积分还是一重积分都一样的）

1. 确认积分的上下限
2. 确认积分范围
3. 后续计算

一定不可以说偷懒，然后忽略了前两步。很多时候都是错在确认积分上下限上，其实也就是积分的范围。

根据经验，**如果感觉积分区域题目写的不明确，或者自己画出来怪怪的，那么一定是自己理解错了！**

例如很多题都是从 0 积分到 pi/2，但是有些题就是从 pi/4 积分到 pi/3 之类的。

此外，还需要注意不要把后者想当然在计算积分的时候抄成前者。

### 求导函数是否连续

本来这是导数的题，但是套了一层不定积分之后就不会辨析了。

先定义求出单点导数，然后不会做了。

事实上，接下来就应该是先把挖掉这个点的导数解出来，然后求极限看看是不是为单点导数。

![张宇基础 1000 2 刷错题重刷第 9 章错题](./assets/p18.png)
...
如果 Sa，Sb 很难算，那么可以计算 Sa+Sb 和 Sa。
...
需要注意记号问题，fxx，f'x 等记号的含义。

做关于多元函数偏导相关的问题，搞清楚记号的含义，这类题都不会难。

#### 先代后求与先求后代

先代后求：如果要求某个点的偏导，假设求 fx，那么可以先把 y0 点代入，然后直接对 x 求偏导即可。

先求后代：先按照多元函数偏导公式法求出偏导，然后代入点，得到偏导值。
...
% 对 x 的偏导数公式
\frac{\partial z}{\partial x} = -\frac{F_x(x, y, z)}{F_z(x, y, z)} = -\frac{\frac{\partial F}{\partial x}}{\frac{\partial F}{\partial z}}

% 对 y 的偏导数公式
\frac{\partial z}{\partial y} = -\frac{F_y(x, y, z)}{F_z(x, y, z)} = -\frac{\frac{\partial F}{\partial y}}{\frac{\partial F}{\partial z}}

一定要注意负号。

#### 多元函数偏导隐函数求导

隐函数求导主要的问题就是要注意对每个变量都要判断是否要求导，本身是否为常数。

比较典型的例子是 $f(x+y,xy)$，求 f 对 x 的二阶偏导等。

#### 多元函数偏导全微分法

全微分法适用于给定非常多方程求两个系数之间的关系。

然而在使用这个方法之前，需要先考虑用其他两种方法。

#### 偏积分

主要是两个考点，一个是如果 $f(x, y)$ 偏导数连续，那么 dp/dy = dQ/dx。

另一个是可以使用类似求一元函数积分的方法求解。

### 多元函数微分

一元函数的微分和导数基本一致，所以没什么好考的。但是多元函数微分很有的考。

本质是因为多元函数的偏导是对 x 轴，y 轴求偏导，而多元函数的微分是对于所有点来说的，条件更强。

具体而言，偏导连续可以推导出可微，可微可以推导出偏导 / 连续。

而偏导存在推导不出来连续，连续也推导不出来偏导存在。

对于这类题，最重要的是记住形式，并且套上去就好了。

### 多元函数极值

#### 无条件极值

判断条件为，当 $AC - B^2 > 0$ 时，有极值，且当 $A < 0$ 时，有极大值，当 $A > 0$ 时，有极小值。

需要注意的是，当 AC - B^2 = 0 时，该方法失效，需要使用其他方法，一般不会出现这种情况。

当 AC - B^2 < 0 时，没有极值。

#### 多元函数经典反例

无外乎都是 $f(x, y)=x^m+y^n$，中间的符号可以变，总体的符号也可以变，$m=n$。

#### 拉格朗日乘数法

假设条件为 $f(x, y)=0$，令 $F(x, y, \lambda)=0$，然后分别对 $x, y, \lambda$ 求偏导，得到三个方程，联立求解。
...
f(x,y) 恒等于 0，然后结合语文理解。

## 第 14 章 - 二元函数积分学

### 二重积分积分中值定理

和一重积分类似的，若 $f(x, y)$ 在闭区域 $D$ 上连续，且 $D$ 的面积记为 $S_D$，则在 $D$ 内至少存在一点 $(\xi, \eta)$，使得：

$$
\iint_D f(x, y) \, \mathrm{d}\sigma = f(\xi, \eta) \cdot S_D
$$

本质是将整个曲顶柱体的体积用一个“平均高”乘以“底面积”来等价替代。

1. 极限题中脱积分：求区域收缩到单点的极限（例如当区域的直径 $d = (x^2+y^2)^{1/2} \to 0$ 时），利用积分中值定理把积分号脱掉，化为点值乘以面积，与分母的面积项消去后再求极限。
2. 证明题中若出现积分与某一点函数值的关系，往往直接考虑积分中值定理或与微分中值定理结合使用。

### 二重积分的对称性

利用对称性化简二重积分是考研中最核心的降维技巧，做题前必须优先观察。

#### 关于某个轴对称

假设区域 $D$ 关于某坐标轴对称，记对称后的一半区域为 $D_1$：

1. 关于 $y$ 轴对称（$x$ 的正负对称）：
   - 若 $f(-x, y) = -f(x, y)$（对 $x$ 为奇函数），则 $\iint_D f(x, y) \, \mathrm{d}\sigma = 0$。
   - 若 $f(-x, y) = f(x, y)$（对 $x$ 为偶函数），则 $\iint_D f(x, y) \, \mathrm{d}\sigma = 2 \iint_{D_1} f(x, y) \, \mathrm{d}\sigma$。
2. 关于 $x$ 轴对称（$y$ 的正负对称）：
   - 若 $f(x, -y) = -f(x, y)$（对 $y$ 为奇函数），积分为 $0$。
   - 若 $f(x, -y) = f(x, y)$（对 $y$ 为偶函数），积分翻倍。

做题技巧：被积函数往往是奇偶混合项（例如 $x^3 + x^2 y + y^2$），必须利用积分的线性可加性拆开算，奇函数项直接扔掉写 $0$。

#### 关于某个点对称

若区域 $D$ 关于原点对称（即 $(x, y) \in D \iff (-x, -y) \in D$）：

- 若 $f(-x, -y) = -f(x, y)$，则积分为 $0$。
- 若 $f(-x, -y) = f(x, y)$，则积分为对称区域一半的两倍。

考研中常考奇项，被积函数整体满足 $f(-x, -y) = -f(x, y)$ 时立刻得到结果为 $0$。

#### 轮换对称

若区域 $D$ 的边界方程中，将 $x$ 与 $y$ 互换后区域保持完全不变（即区域关于直线 $y = x$ 对称）：
...
- 识别区域具有 $x, y$ 对易不变性（如圆形 $x^2 + y^2 \le R^2$、正方形等）

- 将原积分 $I$ 改写为：

  $$
  I = \frac{1}{2} \iint_D [f(x, y) + f(y, x)] \, \mathrm{d}\sigma
  $$

- 特别适用于分子分母对称的分式化简，例如被积函数为 $\frac{x}{x+y}$ 时，相加直接化为 $1$，瞬间秒杀。

### 二重积分交换积分次序

交换积分次序的本质就是把“$X$ 型区域”转化为“$Y$ 型区域”（或反之）。

适用场景：内层积分积不出来（原函数不可求，例如含 $e^{x^2}$、$\sin(y^2)$、$(1+x^3)^{1/2}$ 等）。

解题流程：

1. 画图：根据给定的累次积分的上下限，反写出不等式边界，把积分区域 $D$ 准确画出来，求出所有交点坐标。
2. 切刀换向：
   - 原积分先对 $y$ 后对 $x$（先坚切，后平移）：改为先横切（对 $x$ 积），看左边界到右边界，再上下平移确定 $y$ 的范围。
3. 分块拼接：如果换向后穿入线或穿出线的函数表达式发生变化，必须将积分区域切分为多块，分别列出累次积分相加。

### 二重积分极坐标变换

公式：
$$
x = r\cos\theta, \quad y = r\sin\theta, \quad \mathrm{d}\sigma = r \, \mathrm{d}r\mathrm{d}\theta
$$

千万注意：积分微元里面多乘的那个雅可比行列式因子 $r$，漏掉必扣全部分数！

1. 积分区域 $D$ 的边界含有圆弧、圆环、扇形或阿基米德螺线等。
2. 被积函数含有 $x^2 + y^2$ 或 $[(x^2 + y^2)]^{1/2}$ 这类圆对称结构。

解题方法：

1. 定 $\theta$ 范围：从原点出发引射线，扫过整个区域的最小倾角 $\theta_1$ 到最大倾角 $\theta_2$。
2. 定 $r$ 范围：在 $\theta$ 范围内任作一条射线，从原点向外穿过区域，写出穿入边界 $r_1(\theta)$ 与穿出边界 $r_2(\theta)$。
3. 典型方程转化：
   - 圆心在原点的圆 $x^2 + y^2 = R^2 \implies r = R$
   - 偏心圆 $x^2 + y^2 = 2ax \implies r = 2a\cos\theta$
   - 偏心圆 $x^2 + y^2 = 2by \implies r = 2b\sin\theta$
...
注意，无论是反函数化简，除以 0，还是拆根号，都要考虑多种情况！！！

![张宇基础 1000 题第 14 章](./assets/p19.png)
